...
public class Dijkstra {
    ...
    private final PriorityQueue<Nodo> cola = new PriorityQueue<>(); // priority queue propia de Java, usamos el
    ...
    public void dijkstra(int inicial, int tiempoMinutos, int velocidad) {
        ...
        while (!cola.isEmpty()) { // Mientras cola no este vacia
            actual = cola.element().primero; // Obtengo de la cola el nodo con menor peso, en un comienzo será el
                                             // inicial
            ...
            for (int i = 0; i < adyacencia.get(actual).size(); ++i) { // reviso sus adyacentes del vertice actual
                ...
                if (estaBloqueada)
                    ...
                    peso = adyacencia.get(actual).get(i).segundo; // peso de la arista que une actual con adyacente (
                                                                  // actual , adyacente )
    ...
    private void imprimeRuta(int destino, Ruta ruta, int tipo) {
        ...
        if (previo[destino] != -1) {
            ...
            if (tipo == 1)
                ruta.agregarNodo(destino);
            else
                ruta.agregarNodoR(destino);
        }
